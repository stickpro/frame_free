...
class ControllerExtensionModuleSTKNewsCarousel extends Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('stk_news_carousel', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/stk_news_carousel', 'user_token=' . $this->session->data['user_token'], true)
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => strip_tags($this->language->get('heading_title')),
				'href' => $this->url->link('extension/module/stk_news_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true)
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/stk_news_carousel', 'user_token=' . $this->session->data['user_token'], true);
		} else {
			$data['action'] = $this->url->link('extension/module/stk_news_carousel', 'user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id'], true);
		}
    ...
    if (file_exists( DIR_APPLICATION . 'model/blog/article.php' )) {
      ...
      if ($data['is_blog']) {
		...
		$this->response->setOutput($this->load->view('extension/module/stk_news_carousel', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/stk_news_carousel')) {
